...
public:
    ...
    //- public methods
        ...
        /// @brief True only when a thermal interaction exists AND
        /// radiation is enabled in the interaction dictionary. Lets
        /// callers skip radSumTemp()/radNumPrt() entirely when
        /// radiation is off, rather than distributing a zero-filled
        /// buffer nobody uses.
        inline
        bool hasRadiation() const
        {
            if (demSystem_) return demSystem_->hasRadiation();
            return false;
        }
        ...
        /// @brief Per-particle solid-side reaction heat: (1-η)×Q_rxn [W].
        inline
        ...
        /// @brief Per-particle fluid-side reaction heat: η×Q_rxn [W].
        ///        Zero-filled when η = 0 (default for surface reactions).
        inline
        ...
        /**
         * @brief Returns the DEM gas species names as a standard C++ vector.
         *
         * Clean C++ interface. No OpenFOAM dependencies here to maintain
         * standalone DEM capability. Used by buildSpeciesMapping().
         */
        inline std::vector<std::string> gasSpeciesNames() const
        {
        ...
        // getDataFromDEM(), and the other way via
        // sendGasConcentrationsToDEM() below -- there is no separate
        // CFD-to-DEM reaction-data send this class needs to expose.
